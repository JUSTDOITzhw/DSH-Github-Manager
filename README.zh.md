...
面板的入口是**侧边栏底部的 GitHub 徽标**（「设置」上面那一行）。那一行是 `sidebar.footer.action`，`dsh-cost-meter` 的余额卡片也在同一行里 —— 所以徽标按**内容宽度**排布，空间不够时把账号名省略成 `@JUSTDO…`，而不是去占满整行。占满整行会把余额卡片挤出侧边栏列（外壳对那一列有 `overflow:hidden`），直接看不见。侧边栏折叠成 56px 窄栏时徽标只留图标（约 16px），因为窄栏留给第二个入口的空间只有十几像素。
