...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
     ...
     * beside Settings. Every shipped occupant of that row — the Cordis panel
     * button and dsh-cost-meter's balance stack — is a full-width (`width:100%`)
     * block, which is the tell that the row is really a vertical stack of lines
     * that happens to be laid out horizontally. Two such blocks in one row
     * cannot both be full width: the shell clips the column
     * (`.sidebarCol{overflow:hidden}`), so whichever one refuses to shrink
     * pushes the other out of the sidebar entirely.
     *
...
.dsh-gm-layer{flex:0 0 auto;min-width:0;max-width:100%;align-items:center;width:100%;height:42px;margin:0;display:flex;position:relative}
...
`;
    ...
    function Badge(props) {
       ...
       * (a disabled plugin must not leave the shell misshapen). */
      React.useEffect(() => {
        const row = layer.current === null ? null : layer.current.parentElement;
        return foldFooterRow(row, wide) ?? undefined;
      }, [wide]);
