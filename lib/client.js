...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
     ...
     * Styling
     * ------------------------------------------------------------------ */
    /* `sidebar.footer.action` is a `kind:list` slot rendered into one flex row
     * beside Settings, and dsh-cost-meter parks its balance card in the same
     * row. The shell clips that column (`.sidebarCol{overflow:hidden}`), so an
     * action that claims the whole row does not merely crowd its neighbours —
     * it pushes them out of the column and they vanish. Our badge therefore has
     * to *share* the row: it shrinks (`flex:0 1 auto`, `min-width:0`, ellipsis
     * on the label) and never asks for a fixed track. For the same reason the
     * rail variant is icon-sized: a 56px rail minus its 10px padding leaves
     * 36px, of which the balance chip already takes 40px. */

    const CSS = `
.dsh-gm-badge{width:auto;max-width:100%;min-width:0;height:42px;color:var(--dsw-alias-label-primary);cursor:pointer;background:0 0;border:none;border-radius:12px;align-items:center;gap:8px;margin:0;padding:0 10px 0 8px;font-family:inherit;font-size:14px;display:inline-flex;overflow:hidden}
...
.dsh-gm-layer{flex:0 1 auto;min-width:0;max-width:100%;align-self:center;align-items:center;width:auto;height:42px;margin:0 4px;display:flex;position:relative}
...
`;
    ...
    function Panel(props) {
      ...
      /* The panel is a frame-wide surface, so it is positioned from the slot
         ...
         badge: the badge is content-sized and shares that row with other
         plugins' actions, so its left edge moves with the registration order
         while the row's does not. */
      React.useEffect(() => {
        if (typeof document === "undefined") return;
        const host = document.querySelector("[data-dsh-github-manager-anchor]");
        if (host === null) return;
        const row = host.parentElement;
        const target = row !== null && row.getBoundingClientRect().width > 0 ? row : host;
